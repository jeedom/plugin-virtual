...
Lors de l'utilisation d'un équipement qui ne possède pas de retour d'état et si cet équipement est commandé seulement par jeedom, il est possible d'avoir un retour d'état virtuel. Il suffit pour cela de créer un virtuel qui reprend les commandes (Ex: On & Off) de l'équipement et d'ajouter une commande de type info pour gérer l'état virtuel.

image::../images/virtual16.png[]

Il faut mettre le nom de la commande info créée précédemment dans la case "Nom Information" de chaque commande et mettre dans la case valeur la commande multiple : _Valeur-Info_ `&&` _Action-Equip_.
